...
from functools import reduce  # 函数式编程工具
...
from torch.optim.lr_scheduler import MultiStepLR, CosineAnnealingLR, StepLR, ReduceLROnPlateau  # 学习率调度器
...
def get_ip():
    """
        获取本机IP地址
        原理：通过连接一个外部服务器(8.8.8.8)来获取本机出口IP
        """
...
def dynamic_import(from_str, import_str):
    """
        ...
        示例：
            model = dynamic_import("models", "ResNet50")
        """
...
def gen_triplets(labels, ref_labels=None):
    """
        ...
        参数：
            labels: 主标签矩阵 [n1, C] 或 [n1]
            ref_labels: 参考标签矩阵 [n2, C] 或 [n2]（可选）
        返回：
            anc_idxes: anchor索引
            pos_idxes: positive索引
            neg_idxes: negative索引
        """
    # 1. 计算相似性矩阵
    if ref_labels is None:
        # 在同一样本集内生成三元组
        sames = (labels @ labels.T) > 0  # [N, N]
    else:
        # 在两个不同样本集间生成三元组
        sames = (labels @ ref_labels.T) > 0  # [N, M]
    diffs = ~sames
    if ref_labels is None:
...
def redist(inputs, labels):
    batch_size = labels.shape[0]
    n_try = 50
    for _ in range(n_try):
        idx_list = list(range(batch_size))
        swap_cnt = 0
        for now_ in range(batch_size):
            next_ = (now_ + 1) % batch_size
            now_idx = idx_list[now_]
            next_idx = idx_list[next_]
            now_t = labels[now_idx, :]
            next_t = labels[next_idx, :]
            if (now_t * next_t).sum() > 0:
                next_next_ = (next_ + 1) % batch_size
...
def get_centroids(labels, proxies):
    """
        ...
        参数：
            labels: [batch_size, n_classes] 多热编码标签
            proxies: [n_classes, dim] 各类别的代理向量

        返回：
            centroids: [batch_size, dim] 每个样本的加权代理中心
        """
    ...
    if multi_thread and (epoch + 1) != args.n_epochs and early_stopping.counter < early_stopping.patience - 1:
        ...
        if available_gpu:
            ...
            validation_thread = threading.Thread(
                target=validate_fnc,
                args=(args, query_loader, dbase_loader, early_stopping, epoch),
                kwargs=kwargs
            )
...
class EarlyStopping:
    ...
    def __init__(self, patience=10, best_epoch=0, best_map=0.0):
        ...
        self.best_checkpoint = None  # 保存最佳模型状态
        self.early_stop = False  # 是否早停标志
        self.counter = 0  # 不提升计数
...
def gen_test_data(B, C, K, is_multi_hot=False, normalize_embeddings=True, requires_grad=True, device="cpu"):
    ...
    参数：
        ...
        C: 类别数
        K: 嵌入维度
        is_multi_hot: 是否生成多热标签
        normalize_embeddings: 是否对嵌入进行L2归一化
        requires_grad: 嵌入是否需要梯度
        device: 设备
    返回：
        embeddings: [B, K] 嵌入特征
        singles: [B,] 单标签ID（如果是多标签则为None）
        onehots: [B, C] 多热编码标签
    """
    ...
    if is_multi_hot:
        ...
        singles = torch.randint(low=0, high=C, size=[B]).to(device)
...
def build_optimizer(optim_type, parameters, **kwargs):
    """
        ...
        支持类型：
            sgd, rmsprop, adam, amsgrad, adamw
        """
...
def calc_learnable_params(*args):
    """
        ...
        参数：
            *args: 一个或多个nn.Module对象

        返回：
            所有参数中requires_grad=True的总数
        """
    n_parameters = 0
    for x in args:
        if x:
...
def predict(net, dataloader, out_idx=None, use_sign=True, verbose=True, usage=""):
    ...
    参数：
        net: 模型
        dataloader: 数据加载器
        out_idx: 输出索引（用于多输出模型）
        use_sign: 是否对输出取符号（得到二进制哈希码）
        verbose: 是否显示进度条
        usage: 用途描述（用于进度条）
    ...
    with torch.no_grad():
        for batch in data_iter:
            out = net(batch[0].to(device))  #前向传播
            #处理多输出模型
            if out_idx is None:
                embs.append(out)
            ...
            labs.append(batch[-2])  #标签通常在倒数第二个位置
...
def mean_average_precision(qB, rB, qL, rL, topk=None):
    """
    计算平均精度均值(mAP)，深度哈希检索的核心指标

    参数：
        qB: 查询集哈希码 [N_q, K]
        rB: 检索集哈希码 [N_r, K]
        qL: 查询集标签 [N_q, C] 或 [N_q]
        rL: 检索集标签 [N_r, C] 或 [N_r]
        topk: 只考虑前k个检索结果
    ...
    流程：
        ...
        4. 平均所有查询的精度
    """
    N, K = qB.shape  # N=查询数量，k=哈希码长度

    if topk is None:
        topk = rL.shape[0]  #默认考虑全部
    ...
    # 2. 计算汉明距离 （基于内积，0.5*（K - A·B))
    hamming_dists = 0.5 * (K - qB @ rB.T)
    # 3. 计算每个查询的AP
    ap_sum = hamming_dists.new_tensor(0)  #初始化AP总和
    for i in range(N):
...
def NDCG(qF, rF, qL, rL, what=0, k=-1):
    ...
    G = 2**Rel - 1  # 增益
    D = np.log2(2 + np.arange(k))  # 折损因子

    # 根据相似度排序
    if what == 0:
        Rank = np.argsort(1 - cos(qF, rF))  # 余弦相似度
    elif what == 1:
        Rank = np.argsort(hamming(qF, rF))  # 汉明距离
    elif what == 2:
        Rank = np.argsort(euclidean(qF, rF))  # 欧氏距离

    # 计算NDCG
    _NDCG = 0
    for g, rnk in zip(G, Rank):
        dcg_best = (np.sort(g)[::-1][:k] / D).sum()  # 理想DCG
        if dcg_best > 0:
            dcg = (g[rnk[:k]] / D).sum()  # 实际DCG
            _NDCG += dcg / dcg_best
